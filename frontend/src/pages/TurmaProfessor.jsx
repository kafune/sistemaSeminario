...
import {
  ...
  MenuItem, Snackbar, Tab, Tabs, Table, TableBody, TableCell, TableContainer,
...
import {
  CabecalhoPagina, CardMetrica, DialogoConfirmacao, EstadoErro, EstadoVazio,
  LinkVoltar, StatusBadge, TituloDialogo, cardSx, useDialogoTelaCheia,
  useTelaDesktop
} from '../ui'
  ...
  return (
    <Box>
              ...
              {dados.comunicados.length === 0 ? <EstadoVazio compacto titulo="Nenhum comunicado" descricao="Crie avisos que poderão aparecer no futuro portal do aluno." /> : (
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                  {dados.comunicados.map((comunicado) => (
                    <Box key={comunicado.id} sx={{ p: 1.5, border: `1px solid ${TOV.border}`, borderRadius: TOV.radiusSm }}>
        ...
        <Box>
          ...
          {dados.materiais.length === 0 ? <Box sx={cardSx}><EstadoVazio titulo="Nenhum material anexado" descricao="Adicione arquivos para toda a matéria ou para aulas específicas." /></Box> : (
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: 'repeat(2, minmax(0, 1fr))' }, gap: 1.5 }}>
              {dados.materiais.map((material) => <Box key={material.id} sx={{ ...cardSx, p: 2, display: 'flex', gap: 1.5, alignItems: 'center' }}><Box sx={{ width: 42, height: 42, display: 'grid', placeItems: 'center', bgcolor: TOV.graphiteTint, color: TOV.graphite, borderRadius: TOV.radiusSm, flexShrink: 0 }}><FolderCopyOutlinedIcon /></Box><Box sx={{ minWidth: 0, flexGrow: 1 }}><Typography sx={{ fontWeight: 700, overflowWrap: 'anywhere' }}>{material.titulo}</Typography><Typography sx={{ color: TOV.caption, fontSize: TOV.type.caption, overflowWrap: 'anywhere' }}>{material.nome_arquivo} · {tamanho(material.tamanho)} · {dataHora(material.criado_em)}</Typography></Box><Button size="small" startIcon={baixandoId === material.id ? <CircularProgress size={14} /> : <CloudDownloadOutlinedIcon />} disabled={baixandoId === material.id} onClick={() => baixarMaterial(material)}>{telaDesktop ? 'Baixar' : ''}</Button></Box>)}
            </Box>
          )}
        </Box>
      )}
      ...
      <Dialog open={!!aulaPlanejar} onClose={salvandoPlano ? undefined : () => setAulaPlanejar(null)} maxWidth="md" fullWidth fullScreen={dialogoTelaCheia}>
        <TituloDialogo onFechar={salvandoPlano ? undefined : () => setAulaPlanejar(null)}>Planejamento da aula · {aulaPlanejar ? dataCurta(aulaPlanejar.data) : ''}</TituloDialogo>
      ...
      <Dialog open={comunicadoAberto} onClose={salvandoComunicado ? undefined : () => setComunicadoAberto(false)} maxWidth="sm" fullWidth fullScreen={dialogoTelaCheia}>
        <TituloDialogo onFechar={salvandoComunicado ? undefined : () => setComunicadoAberto(false)}>{comunicadoEditando ? 'Editar comunicado' : 'Novo comunicado'}</TituloDialogo>
